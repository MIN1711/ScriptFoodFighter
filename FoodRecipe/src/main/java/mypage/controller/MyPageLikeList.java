...
@WebServlet("/customer/myRecomlist.do")
public class MyPageLikeList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		// cusId parameter 값을 받아온다.
				...
				MyRecomVO rv = new MyRecomVO();
				rv.setCusId(cusId);
				
				// 회원이 작성한 글의 전체 갯수를 조회한다.
				int countList = service.countList(rv);
				...
				// 회원이 작성한 글 목록을 조회한다.
				List<MyRecomVO> mylikeList = service.myRecomList(map);
				...
				req.setAttribute("list", mylikeList);
				...
				req.getRequestDispatcher("/myPageRecomList.jsp").forward(req, res);
	}
