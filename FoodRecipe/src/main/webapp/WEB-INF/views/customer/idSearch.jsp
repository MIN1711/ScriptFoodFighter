...
<% 
 	...
 	//System.out.println(cv.getcusTel());
%>
<!DOCTYPE html>
<html>
<head>
...
<form class="form" onsubmit="return false" autocomplete="off">
  <div class="form-inner">
	...
	<label for="login-password" >이름 입력</label>
	...
	<div class="input-wrapper">
	<label for="login-password" >번호 입력</label>
	<div class="input-group">
        <span class="icon">
          <svg viewBox="0 0 24 24"></svg></span>
	...
	<div class="btn-group">
	<button id="search" class="btn btn--primary">찾기</button>
...
$('#search').on('click', function(){
    ...
    $.ajax({
        url: '<%=request.getContextPath()%>/customer/idSearch.do',
        type: 'post',
        data: {"cusName": cusName, "cusTel": cusTel},
        dataType: 'json',
        success: function(resp) {
        	console.log(resp)
            if (resp.code == 'OK') {
                alert("회원 아이디는 " + resp.data + "입니다.");
            } else {
            	$('#cusName').val("존재하지 않는 회원입니다.");
